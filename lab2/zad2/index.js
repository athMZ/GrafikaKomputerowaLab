...
function installMouseHandler() {
    ...
    function doMouseMove(evt) {
        ...
        if (shapeChoice == 0) {
            ...
            const polygonSidesNumber = 9;
            const radius = 20;

            let points = [];

            for (let i = 0; i < polygonSidesNumber * 2; i += 2) {
                points[i] = x + (radius * Math.cos((Math.PI / polygonSidesNumber) * i));
                points[i + 1] = y + (radius * Math.sin((Math.PI / polygonSidesNumber) * i));
            }
