...
public class Lab5 extends GLJPanel implements GLEventListener, KeyListener{
	...
	private void draw(GL2 gl2) {
		...
		if(objectNumber == 1)
			drawCorkscrew(gl2, 5, 0, -10, 0);
		else if(objectNumber == 2)
			Pyramid(gl2);
		
	}

	public void drawCorkscrew(GL2 gl2, int n, double xOffset, double yOffset, double zOffset) {
        ...
        var angle = 2 * Math.PI / (2 * n);

        for(float i = 0; i < scale * n; i++)
        {
            x = Math.cos(angle * i) * (0.05 * i) + xOffset;
            z = Math.sin(angle * i) * (0.05 * i) + zOffset;
            y = (i / (density * n)) + yOffset;
	...
	public void Traingle(GL2 gl2, float start, float space, float n) {
        gl2.glColor3f(0, 1, 0);
        gl2.glBegin(GL_TRIANGLE_FAN);

        float height = 2.5f;
        
        float x1 = (float)(Math.cos(start * 2 * Math.PI / n));
        float y1 = (float)(Math.sin(start * 2 * Math.PI / n));
        gl2.glVertex3d(x1, y1, height);
        
        float x2 = (float)(Math.cos((start + space) * 2 * Math.PI / n));
        float y2 = (float)(Math.sin((start + space) * 2 * Math.PI / n));
        
        gl2.glVertex3d(x2, y2, height);
